...
source("models/VECM/synthetic_normal_paramertest.R")
...
# Data Frame
data.group1 <- GenerateSampleData(pi[[1]], num.total.days)
data.group2 <- GenerateSampleData(pi[[2]], num.total.days)
data.group3 <- GenerateSampleData(pi[[3]], num.total.days)
...
sigma.hat.group1 <- fit.result.group1[[2]]
sigma.hat.group2 <- fit.result.group2[[2]]
sigma.hat.group3 <- fit.result.group3[[2]]
#========= Group Exp and Var ============
...
est.mean <- list()
for(i in 1: 1000){
  est.mean.group1 <- EstimateMean(pi.hat.group1[[i]],num.test.days,initial.price.group1)
  est.mean.group2 <- EstimateMean(pi.hat.group2[[i]],num.test.days,initial.price.group2)
  est.meam.group3 <- EstimateMean(pi.hat.group3[[i]],num.test.days,initial.price.group3)
...
est.var <- list()
for(i in 1:1000){
  est.var.group1 <- EstimateVar(pi.hat.group1[[i]],sigma.hat.group1[[i]], 5)
  est.var.group2 <- EstimateVar(pi.hat.group2[[i]],sigma.hat.group2[[i]], 5)
  est.var.group3 <- EstimateVar(pi.hat.group3[[i]],sigma.hat.group3[[i]], 5)
...
for(i in 1:1000){
  initial.price.group1 <- data.group1[[i]][100,]
  initial.price.group2 <- data.group2[[i]][100,]
  initial.price.group3 <- data.group3[[i]][100,]
  initial.price[i,] <- c(initial.price.group1,initial.price.group2,initial.price.group3)
  final.price.group1 <- data.group1[[i]][100+num.test.days,]
  final.price.group2 <- data.group2[[i]][100+num.test.days,]
  final.price.group3 <- data.group3[[i]][100+num.test.days,]
  final.price[i,] <- c(final.price.group1,final.price.group2,final.price.group3)
}

#========Get final Return =============
finalStep <- function(gamma,est.mean,est.var){
  opt.weight <- GetWeight(est.mean, est.var, gamma)
  final.return <- c(0, ncol = 1000, nrow = 8)
  for(i in 1: 1000){
...
final.density<- list()
for(i in 1:8){
  return<-finalStep(gamma[i],est.mean, est.var)
  final.density[[i]] = density(return)
}

# Plot density graphs
plot(final.density[[1]], type = 'l', col = '1', main = 'Estimated parameters', xlab = 'Expected Reward',ylim = c(0,0.7))
for (i in 2:8) {
...
#========== Group Variance ==========
  true.var.group1 <- EstimateVar(pi[[1]],R, 5)
  true.var.group2 <- EstimateVar(pi[[2]],R, 5)
  true.var.group3 <- EstimateVar(pi[[3]],R, 5)
...
# Plot density graphs
plot(final.density.true[[1]], type = 'l', col = '1', main = 'True parameters', xlab = 'Expected Reward',ylim = c(0,0.7))
for (i in 2:8) {
