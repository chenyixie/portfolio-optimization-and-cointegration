#Portfolio optimization for synthetic data _ markowitz_normal_dep
library(MASS)
...
source("models/markowitz/normal_dependent.R")
...
test.data <- list()
for(i in 1:1000){
  all.data[[i]] <- GenerateData(mu, sigma, init.price, num.days)
...
# Plot density graphs
plot(final.density[[1]], type = 'l', col = '1', ylim = c(0,0.3), main = 'Estimated parameters', xlab = 'Expected Reward')
for (i in 2:8) {
...
#Plot density graphs
plot(final.true.density[[1]], type = 'l', col = '1', ylim = c(0,0.3),main = 'True parameters', xlab = "Expected Reward")
for (i in 2:8) {
