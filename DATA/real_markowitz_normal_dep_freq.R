...
#
#        d(y_t) = mu + e_t
#        mu.hat = mean(d(y_t)) (9*1 vector)
#        sigma.hat = mean( (d(y_t)-mu.hat)%*% t(d(y_t)-mu.hat)) (9*9 matrix)
#        epsilon_t = N(0,sigma.hat) (9*1 vector)
#        mean(d(y_t+s - y_t)) = mu.hat * s (9*1 vector)
#            y_20 - y_0 = (y_20 - y_19) + ... + (y_1 - y_0) = mu + mu + ... + mu = mu * 20
#        var(d(y_t+s - y_t)) = sigma.hat^2 * s (9*9 covariance matrix)
#            var(y_20 - y_0) = 
#
...
# run all windows using one gamma
test1 <- function(all.data) {
  results <- RunAllWindows(all.data, 0.1, 100, 20)
  all.final.return <- results[[1]]
  all.rsqure <- results[[2]]
  
  par(mfrow = c(1,1))
  plot(all.final.return, type = 'l')
...
test2 <- function(all.data, num.train.days, num.test.days) {
  ...
  all.data.slice <- SliceData(all.data, 1, num.train.days + num.test.days)
  result <- RunOneWindow(all.data.slice, gamma, num.train.days, num.test.days)
  final.return <- result[[1]]
  all.rsquare <- result[[2]]
...
test4 <- function(all.data) {
  ...
  cat(sprintf("  daily: mean return = %.5f, total return = %.5f\n", return.mean, return.sum))
  ...
  cat(sprintf(" weekly: mean return = %.5f, total return = %.5f\n", return.mean.week, return.sum.week))
  ...
  cat(sprintf("monthly: mean return = %.5f, total return = %.5f\n", return.mean.month, return.sum.month))
}

# test new multivariate model
test5 <- function(all.data) {
  all.data.slice <- SliceData(all.data, 1, 120)
  train.data <- all.data.slice[1:100, ]
  fit.result <- FitModel(train.data)
  mu.hat <- fit.result[[1]]
  sigma.hat <- fit.result[[2]]
  res <- fit.result[[3]]
...
# test all gammas by different frequency
